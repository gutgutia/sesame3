...
export function Sidebar() {
  ...
  const [planTier, setPlanTier] = useState<"free" | "paid" | null>(null);
  const dropdownRef = useRef<HTMLDivElement>(null);
  const showUpgradeUI = useShowUpgradeUI();

  // Check if user is admin and get plan tier
  useEffect(() => {
    fetch("/api/user/me")
    ...
    fetch("/api/settings")
      ...
      .then((data) => {
        if (data?.subscription?.tier) {
          setPlanTier(data.subscription.tier === "paid" ? "paid" : "free");
        } else {
          setPlanTier("free");
        }
      })
  ...
  return (
    <aside className="hidden md:flex w-[260px] bg-sidebar border-r border-border-subtle p-6 flex-col sticky top-0 h-screen">
      ...
      <a
        ...
        className="flex items-center gap-2 px-3 py-1.5 mb-3 text-xs text-text-muted transition-all hover:text-text-main"
      >
